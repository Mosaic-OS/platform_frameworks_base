...
public class LockSettingsService extends ILockSettings.Stub {
    ...
    @GuardedBy("itself")
    private final SparseIntArray mUserCredentialTypesPrimary = new SparseIntArray();

    @GuardedBy("itself")
    private final SparseIntArray mUserCredentialTypesSecondary = new SparseIntArray();
    ...
    private void tieProfileLockIfNecessary(int profileUserId,
        ...
        if (!isUserSecure(parent.id) && !profileUserPassword.isNone()) {
            ...
            clearUnifiedProfilePassword(profileUserPassword,
    ...
    static class Injector {
        ...
        public SyntheticPasswordManager getSyntheticPasswordManager(
            ...
            return new SyntheticPasswordManager(
                    getContext(), storage, getUserManager(), new PasswordSlotManager(), keyStore, false);
        }
    ...
    private boolean isUseOneLockSettingEnabledInternal(int userId) {
        if (getCredentialTypeInternal(userId, Primary) != CREDENTIAL_TYPE_NONE) {
    ...
    private int getCredentialTypeInternal(int userId, LockDomain lockDomain) {
        ...
        int credentialType = queryServiceSideCredentialTypeCache(userId, lockDomain);
        if (credentialType == CREDENTIAL_TYPE_UNKNOWN) {
            ...
            synchronized (mSpManager) {
                credentialType = getCredentialTypeInternalLockedUncached(userId, lockDomain);
                updateServiceSideCredentialTypeCache(userId, lockDomain, credentialType);
            }
        }
    ...
    @GuardedBy("mSpManager")
    private int getCredentialTypeInternalLockedUncached(int userId, LockDomain lockDomain) {
        if (!checkUserSupportsBiometricSecondFactorIfSecondary(userId, lockDomain)) {
    ...
    @VisibleForTesting
    int queryServiceSideCredentialTypeCache(int userId, LockDomain lockDomain) {
        if (!android.security.Flags.enableServiceSideCredentialTypeCache()) {
            return CREDENTIAL_TYPE_UNKNOWN;
        }
        SparseIntArray map = lockDomain == Primary ? mUserCredentialTypesPrimary : mUserCredentialTypesSecondary;
        return queryServiceSideCredentialTypeCacheInner(map, userId);
    }

    private static int queryServiceSideCredentialTypeCacheInner(SparseIntArray mUserCredentialTypes, int userId) {
    ...
    @GuardedBy("mSpManager")
    private void updateServiceSideCredentialTypeCache(int userId, LockDomain domain, int credentialType) {
        if (!android.security.Flags.enableServiceSideCredentialTypeCache()) {
            return;
        }
        SparseIntArray map = domain == Primary ? mUserCredentialTypesPrimary : mUserCredentialTypesSecondary;
        synchronized (map) {
            map.put(userId, credentialType);
        }
    }
    ...
    private void invalidateServiceSideCredentialTypeCache(int userId) {
        ...
        invalidateServiceSideCredentialTypeCacheInner(mUserCredentialTypesPrimary, userId);
        invalidateServiceSideCredentialTypeCacheInner(mUserCredentialTypesSecondary, userId);
    }

    private static void invalidateServiceSideCredentialTypeCacheInner(final SparseIntArray mUserCredentialTypes, int userId) {
        synchronized (mUserCredentialTypes) {
    ...
    boolean migrateChildProfileLockPasswordToProfileProtectorPwd(
        ...
        synchronized (mSpManager) {
            ...
            if (getCredentialTypeInternal(profileId, Primary) == CREDENTIAL_TYPE_NONE) {
                Slogf.w(
                        TAG,
            ...
            if (isCredentialShareableWithParent(profileUserId)) {
                ...
                if (isSecure) {
                    ...
                    if (profilePasswordMap != null
                        ...
                        clearUnifiedProfilePassword(profilePassword, Primary, profileUserId);
                    } else {
    ...
    private void clearUnifiedProfilePassword(
            LockscreenCredential profilePassword, LockDomain domain, int profileUserId) {
        if (domain != Primary) {
            return;
        }
        setLockCredentialInternal(
    ...
    public boolean setLockCredential(LockscreenCredential credential,
        ...
        try {
            ...
            synchronized (mSeparateChallengeLock) {
                ...
                notifyPasswordMetricsChanged(credential, lockDomain, userId);
            }
            if (isCredentialShareableWithParent(userId)) {
            ...
            onPostLockCredentialChanged(credential, lockDomain, userId);
            return true;
        } finally {
    ...
    private void onPostLockCredentialChanged(LockscreenCredential newCredential, LockDomain lockDomain,
            int userId) {
    ...
    private void notifyPasswordMetricsChanged(
            LockscreenCredential newCredential, LockDomain lockDomain,
        ...
        mHandler.post(
                () -> {
                    mInjector.getDevicePolicyManager().reportPasswordChanged(newMetrics, userId, lockDomain);
                    if (!android.security.Flags.moveNotifyLockCredentialCalls()) {
                        if (lockDomain == Primary) {
        ...
        invalidateCredentialTypeCaches(userId);
         if (lockDomain == Primary) {
            ...
            if (credential.isNone()
                ...
                notifyPasswordMetricsChanged(credential, Secondary, userId);
                onPostLockCredentialChanged(credential, Secondary, userId);
            }
        }
    ...
    private boolean setLockCredentialWithToken(LockscreenCredential credential, long tokenHandle,
        ...
        if (result) {
            ...
            notifyPasswordMetricsChanged(credential, Primary, userId);
            notifySeparateProfileChallengeChanged(userId);
        }
        return result;
    }
     ...
     */
    @VisibleForTesting
    void lockUser(@UserIdInt int userId) {
    ...
    private final class LocalService extends LockSettingsInternal {
        ...
        public boolean setLockCredentialWithToken(LockscreenCredential credential, long tokenHandle,
            ...
            onPostLockCredentialChanged(credential, Primary, userId);
            return true;
        }
