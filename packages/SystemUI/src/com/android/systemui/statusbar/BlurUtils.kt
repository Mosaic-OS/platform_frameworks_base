...
constructor(
    ...
    // Cached so blurRadiusOfRatio() does not do a Settings IPC per frame; -1f means unset.
    ...
    fun blurRadiusOfRatio(ratio: Float): Float {
        ...
        if (customRadius >= 0f) {
            // Clamp so a bad setting cannot push an unbounded blur radius to SurfaceFlinger.
